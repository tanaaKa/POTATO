    class CBA_556x45_AK {
        GVAR(mags)[] = {
            MAGAZINE_WELL_ALT_MAG(556x45,30,ak)
        };
    };             // AK mags for 5.56 AK type rifles, AK-101, AK-102, etc.
    class CBA_556x45_RPK {
        GVAR(mags)[] = {
            MAGAZINE_WELL_MAG(556x45,30),
            MAGAZINE_WELL_MAG(556x45,60),
            MAGAZINE_WELL_MAG(556x45,45)
        };
    };            // 45rnd RPK mags for 5.56 RPK-201
    class CBA_556x45_FAMAS {
        GVAR(mags)[] = {
            MAGAZINE_WELL_MAG(556x45,25)
        };
    };          // FAMAS F1
    class CBA_556x45_GALIL {
        GVAR(mags)[] = {
            MAGAZINE_WELL_MAG(556x45,35),
            MAGAZINE_WELL_MAG(556x45,50)
        };
    };          // IMI Galil
    class CBA_556x45_G36 {
        GVAR(mags)[] = {
            MAGAZINE_WELL_MAG(556x45,20),
            MAGAZINE_WELL_MAG(556x45,30),
            MAGAZINE_WELL_MAG(556x45,60),
            MAGAZINE_WELL_MAG(556x45,100),
            MAGAZINE_WELL_MAG(556x45,150)
        };
    };            // H&K G36
    // class CBA_556x45_HK33 {
    //     GVAR(mags)[] = {
    //         MAGAZINE_WELL_ALT_MAG(556x45,30,hk33)
    //     };
    // };           // H&K 33/53/93
    class CBA_556x45_M21 {
        GVAR(mags)[] = {
            MAGAZINE_WELL_MAG(556x45,20),
            MAGAZINE_WELL_MAG(556x45,30),
            MAGAZINE_WELL_MAG(556x45,60),
            MAGAZINE_WELL_MAG(556x45,100),
            MAGAZINE_WELL_MAG(556x45,150)
        };
    };            // Zastava M21

    class CBA_556x45_MINIMI {
        GVAR(mags)[] = {
            MAGAZINE_WELL_MAG(556x45,30),
            MAGAZINE_WELL_BOX(556x45,100),
            MAGAZINE_WELL_BOX(556x45,200)
        };
    };

    class CBA_556x45_TYPE97 {
        GVAR(mags)[] = {
            MAGAZINE_WELL_MAG(556x45,30),
            MAGAZINE_WELL_MAG(556x45,60)
        };
    };         // QBZ-97 Stick Mags
    class CBA_556x45_TYPE97_XL {
        GVAR(mags)[] = {
            MAGAZINE_WELL_MAG(556x45,100),
            MAGAZINE_WELL_MAG(556x45,150)
        };
    };      // QBB-97 LSW Drums
    class CBA_556x45_SCAR_EGLM {        // SCAR-L with EGLM/FN40GL/Mk 13 Mod 0 grenage launcher (This extends the length of the magwell and prevents mags that are too short or wide from being used)
        GVAR(mags)[] = {
            MAGAZINE_WELL_MAG(556x45,30),
            MAGAZINE_WELL_MAG(556x45,60)
        };
    };
    class CBA_556x45_SG550 {
        GVAR(mags)[] = {
            MAGAZINE_WELL_MAG(556x45,20),
            MAGAZINE_WELL_MAG(556x45,30),
            MAGAZINE_WELL_MAG(556x45,60),
            MAGAZINE_WELL_MAG(556x45,100),
            MAGAZINE_WELL_MAG(556x45,150)
        };
    };

    class CBA_556x45_STANAG {           // STANAG mags, standard length, including small drums
        GVAR(mags)[] = {
            MAGAZINE_WELL_MAG(556x45,20),
            MAGAZINE_WELL_MAG(556x45,30)
        };
    };

    class CBA_556x45_STANAG_L {
        GVAR(mags)[] = {
            MAGAZINE_WELL_MAG(556x45,60)
        };
    };       // STANAG mags, long stick or coffin (40/60 rounds, Magpul PMAG 40, Surefire MAG5-60)
    class CBA_556x45_STANAG_XL {};      // STANAG mags, extra long stick or coffin (80/100 rounds, Surefire MAG5-100)
    class CBA_556x45_STANAG_2D {
        GVAR(mags)[] = {
            MAGAZINE_WELL_MAG(556x45,100)
        };
    };      // STANAG mags, twin drums (100rnd Beta C-MAG)
    class CBA_556x45_STANAG_2D_XL {     // STANAG mags, extra large twin-drums (150rnd Armatac SAW-MAG)
        GVAR(mags)[] = {
            MAGAZINE_WELL_MAG(556x45,150)
        };
    };

    class CBA_556x45_STEYR {
        GVAR(mags)[] = {
            MAGAZINE_WELL_MAG(556x45,20),
            MAGAZINE_WELL_MAG(556x45,30),
            MAGAZINE_WELL_MAG(556x45,42),
            MAGAZINE_WELL_MAG(556x45,60),
            MAGAZINE_WELL_MAG(556x45,100),
            MAGAZINE_WELL_MAG(556x45,150)
        };
    };          // Steyr AUG
